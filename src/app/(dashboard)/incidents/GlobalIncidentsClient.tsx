...
export default function GlobalIncidentsClient({
    ...
    const handleTogglePreference = async (projectId: string) => {
        const currentlyEnabled = prefs[projectId] || false;
        const newEnabled = !currentlyEnabled;
        setTogglingClient(projectId);
        try {
            ...
            setPrefs({ ...prefs, [projectId]: newEnabled });
    ...
    const filtered = incidents.filter(inc =>
        ...
        inc.project?.name.toLowerCase().includes(search.toLowerCase())
    );

    const globalCount = projects.filter(c => c.notificationMode === "global").length;
    const customCount = projects.filter(c => c.notificationMode === "custom").length;
    const disabledCount = projects.filter(c => c.notificationMode === "disabled").length;
            ...
            {activeTab === "overview" ? (
                    ...
                    {filtered.length === 0 ? (
                        ...
                        <div style={{ borderRadius: "10px", overflow: "hidden", border: "1px solid var(--color-border)" }}>
                            ...
                            {filtered.map(inc => {
                                ...
                                return (
                                    <Link key={inc.id} href={`/projects/${inc.projectId}/monitoring/incidents/${inc.id}`} style={{ textDecoration: "none" }}>
                                        <div style={{
                                            display: "grid",
                                            gridTemplateColumns: "1fr 200px 180px",
                                            padding: "16px 20px",
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
                    ...
                    <div style={{
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
                            {projects.map(p => {
                                const mode = getModeLabel(p.notificationMode);
                                return (
                                    <Link key={p.id} href={`/projects/${p.id}/monitoring/incidents?tab=settings`} style={{ textDecoration: "none" }}>
                                        <div style={{
                ...
                <div style={{
                    ...
                    <div style={{ display: "flex", gap: "16px", marginBottom: "20px", marginTop: "16px" }}>
                        ...
                        <div style={{ fontSize: "0.8rem", color: "var(--color-text-muted)", display: "flex", alignItems: "center", gap: "6px" }}>
                            <span style={{ width: "8px", height: "8px", borderRadius: "50%", background: "#9ca3af" }} />
                            {projects.length - Object.values(prefs).filter(Boolean).length} inactief
                        </div>
                    </div>

                    <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
                        {projects.map(p => {
                            const isEnabled = prefs[p.id] || false;
                            const isToggling = togglingClient === p.id;
                            return (
                                <div key={p.id} style={{
                                    display: "flex", alignItems: "center", justifyContent: "space-between",
                                    ...
                                    <button
                                        onClick={() => handleTogglePreference(p.id)}
                                        disabled={isToggling}
                                        style={{
