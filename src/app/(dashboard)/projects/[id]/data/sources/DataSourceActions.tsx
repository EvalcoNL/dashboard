...
export default function DataSourceActions({
    ...
    const handleReconnect = () => {
        const linkRoutes: Record<string, string> = {
            ...
            META: "/api/auth/meta/link",
            MICROSOFT_ADS: "/api/auth/microsoft-ads/link",
            LINKEDIN: "/api/auth/linkedin/link",
            TIKTOK: "/api/auth/tiktok/link",
            PINTEREST: "/api/auth/pinterest/link",
            SLACK: "/api/auth/slack/link",
        };
        const route = linkRoutes[sourceType || ""];
        if (!route) return; // No reconnect route for this type
        window.location.href = `${route}?projectId=${projectId}`;
    };

    const handleSettings = () => {
        router.push(`/projects/${projectId}/data/sources/${sourceId}/edit`);
    };
            ...
            <div ref={dropdownRef} style={{ position: "relative" }}>
                ...
                {isOpen && (
                    ...
                    >
                        ...
                        <button
                            onClick={() => { setIsOpen(false); router.push(`/projects/${projectId}/data/access?sourceId=${sourceId}`); }}
                            style={menuItemStyle}
                            onMouseEnter={(e) => e.currentTarget.style.background = "rgba(255,255,255,0.05)"}
                            onMouseLeave={(e) => e.currentTarget.style.background = "none"}
                        >
