...
const API_KEY_FIELDS: Record<string, { key: string; label: string; placeholder: string; required?: boolean; type?: string }[]> = {
    ...
    SHOPWARE: [
        { key: "domain", label: "Shopware Store URL", placeholder: "https://jouw-webshop.nl", required: true },
        { key: "projectId", label: "Integration Client ID", placeholder: "SWIA...", required: true },
        { key: "clientSecret", label: "Integration Client Secret", placeholder: "Voer je client secret in...", required: true, type: "password" },
    ],
    LIGHTSPEED: [
...
export default function NewDataSourceClient({ projectId, clientName }: NewDataSourceClientProps) {
    ...
    const getAction = (platformKey: string) => {
        ...
        switch (meta.authType) {
            case "oauth_google": {
                ...
                return { href: `/api/auth/${slugMap[platformKey]}/link?projectId=${projectId}` };
            }
            case "oauth_third": {
                const slugMap: Record<string, string> = {
                    META: "meta",
    ...
    const renderCard = (platformKey: string) => {
        ...
        // Coming Soon: render disabled card with badge
        if (meta.comingSoon) {
            return (
                <SourceCard
                    key={platformKey}
        ...
        if (platformKey === "WEBSITE") {
            return (
                <SourceCard
                    key={platformKey}
                    href={`/projects/${projectId}/data/sources/new/domain`}
    ...
    return (
        <>
            <div style={{ padding: "32px", maxWidth: "1200px", margin: "0 auto" }}>
                <Link
                    href={`/projects/${projectId}/data/sources`}
                    style={{
                ...
                <div style={{ marginTop: "48px", paddingTop: "32px", borderTop: "1px solid var(--color-border)" }}>
                    ...
                    <SampleDataManager projectId={projectId} />
                </div>
            </div>
