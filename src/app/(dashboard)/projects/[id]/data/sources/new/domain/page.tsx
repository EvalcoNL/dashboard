...
export default function NewDomainSourcePage({
    params
}: {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch(`/api/projects/${id}/data-sources`, {
                method: "POST",
            ...
            if (!res.ok) {
                ...
                throw new Error(data.error || "Er is een fout opgetreden bij het toevoegen.");
            }

            // Redirect back to data sources overview
            router.push(`/projects/${id}/data/sources`);
            router.refresh();
        } catch (error: any) {
    ...
    return (
        <div style={{ padding: "32px", maxWidth: "800px", margin: "0 auto" }}>
            <Link
                href={`/projects/${id}/data/sources/new`}
                style={{
                    display: "inline-flex",
                    alignItems: "center",
                    gap: "6px",
            ...
            >
                <ArrowLeft size={16} /> Terug naar bronnen selectie
            </Link>
            ...
            <p style={{ color: "var(--color-text-secondary)", marginBottom: "40px" }}>
                Voeg een website toe om de beschikbaarheid en performance te monitoren.
            </p>
