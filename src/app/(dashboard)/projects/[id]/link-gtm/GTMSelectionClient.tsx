...
export default function GTMSelectionClient({
    projectId,
    sourceId,
    accounts,
}: {
    ...
    const handleSelect = async (account: GTMAccount) => {
        ...
        try {
            const res = await fetch(`/api/projects/${projectId}/data-sources/${sourceId}/activate`, {
                ...
                body: JSON.stringify({
                    ...
                    name: account.name || `Google Tag Manager (${account.id})`,
                }),
            });
            if (!res.ok) {
    ...
    if (accounts.length === 0) {
        return (
            <div style={{
                ...
                Geen Google Tag Manager accounts gevonden.
            </div>
        );
    }
