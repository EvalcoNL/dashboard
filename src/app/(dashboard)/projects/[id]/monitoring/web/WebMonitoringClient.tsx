...
export default function WebMonitoringClient({ projectId, clientName, domains, incidents, summary }: WebMonitoringClientProps) {
                ...
                {domains.map(domain => {
                    ...
                    return (
                        <div key={domain.id} className="glass-card" style={{ overflow: "visible" }}>
                            <div
                                onClick={() => router.push(`/projects/${projectId}/monitoring/web/${domain.id}`)}
                                style={{
                                ...
                                <div className="action-menu-container" style={{ position: "relative", marginLeft: "auto" }}>
                                    ...
                                    {openMenuId === domain.id && (
                                        <div style={{ position: "absolute", right: 0, top: "100%", marginTop: "4px", background: "var(--color-surface-elevated)", border: "1px solid var(--color-border)", borderRadius: "8px", boxShadow: "0 8px 24px rgba(0,0,0,0.3)", zIndex: 50, minWidth: "160px", overflow: "hidden" }}>
                                            ...
                                            <button onClick={(e) => { e.stopPropagation(); router.push(`/projects/${projectId}/monitoring/web/${domain.id}`); setOpenMenuId(null); }} className="dropdown-item" style={dropdownItemStyle}><Settings size={14} /> Instellingen</button>
