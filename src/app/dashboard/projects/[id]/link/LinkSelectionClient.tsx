...
export default function LinkSelectionClient({
    ...
    const [done, setDone] = useState<false | "syncing" | "completed">(false);
    ...
    const handleLink = async (account: GoogleAccount) => {
        ...
        try {
            ...
            if (res.ok) {
                setDone("syncing");
                try {
                    // Trigger immediate background sync for this client
                    await fetch(`/api/projects/${clientId}/sync`, { method: "POST" });
                } catch (syncError) {
                ...
                setDone("completed");
                setTimeout(() => {
                    router.push(`/dashboard/projects/${clientId}?linked=success`);
                    router.refresh();
                }, 1500);
            } else {
                setLinking(null);
    ...
    if (done === "syncing" || done === "completed") {
        return (
            <div className="glass-card" style={{ padding: "48px", textAlign: "center" }}>
                <div style={{
                    ...
                    height: "64px",
                    borderRadius: "50%",
                    background: done === "syncing" ? "rgba(99, 102, 241, 0.1)" : "rgba(16, 185, 129, 0.1)",
                    display: "flex",
                    alignItems: "center",
                    justifyContent: "center",
                    color: done === "syncing" ? "#6366f1" : "#10b981",
                    margin: "0 auto 20px"
                }}>
                    {done === "syncing" ? <Loader2 size={32} className="animate-spin" /> : <CheckCircle size={32} />}
                </div>
                <h2 style={{ fontSize: "1.25rem", fontWeight: 700, marginBottom: "8px" }}>
                    {done === "syncing" ? "Data Synchroniseren..." : "Account Gekoppeld!"}
                </h2>
                <p style={{ color: "var(--color-text-secondary)", fontSize: "0.9rem" }}>
                    {done === "syncing"
                        ? "Campagne gegevens worden opgehaald, een moment geduld aub."
