...
export default function DashboardsOverviewClient() {
    ...
    const createDashboard = async (name: string, templateId?: string) => {
        ...
        try {
            const res = await fetch("/api/dashboards", {
                ...
                body: JSON.stringify({ clientId, name, templateId }),
            });
            const data = await res.json();
            if (data.success) {
                router.push(`/dashboard/projects/${clientId}/reports/dashboards/${data.dashboard.id}`);
            }
    ...
    const duplicateDashboard = async (id: string) => {
        ...
        const createRes = await fetch("/api/dashboards", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ clientId, name: `${data.dashboard.name} (kopie)` }),
        });
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: "16px" }}>
                        {dashboards.map(db => (
                            <div
                                ...
                                onClick={() => router.push(`/dashboard/projects/${clientId}/reports/dashboards/${db.id}`)}
                            >
                                {/* Card Header */}
