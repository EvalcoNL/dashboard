...
export class GoogleAdsService {
    private configCache: { data: GoogleAdsConfig; expiry: number } | null = null;
    private static CONFIG_TTL_MS = 60_000; // 60 seconds

    private async getConfig(): Promise<GoogleAdsConfig> {
        ...
        this.configCache = { data: config, expiry: Date.now() + GoogleAdsService.CONFIG_TTL_MS };
        return config;
    }
    ...
    /**
     * Fetches users (email addresses) with access to a specific Google Ads customer account.
     */
    async getAccountUsers(
        ...
        try {
            ...
            return rows
                .filter((row) => row.customer_user_access?.email_address)
                .map((row) => ({
                    email: row.customer_user_access!.email_address!,
                    name: row.customer_user_access!.email_address!.split("@")[0],
                    role: row.customer_user_access!.access_role?.toString() || "UNKNOWN",
                }));
        } catch (err) {
            console.error(`Failed to fetch account users for ${customerId}:`, err);
            return [];
        }
    }
}
